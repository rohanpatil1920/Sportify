...
// const BookingForm: React.FC<BookingFormProps> = ({ pricePerHour }) => {
// const { venueId } = useParams<{ venueId: string }>();
...
//         setAvailableCourts(response.data);
//         if (response.data.length > 0) {
//           setSelectedCourt(response.data[0].id); // Default to the first court
//         } else {
//           setError("No courts available for this venue.");
//         }
//       } catch (ex) {
...
//   useEffect(() => {
//     // Generate available slots and filter past slots based on the selected date
//     const generateSlots = () => {
//       const slots = [];
...
//           {availableCourts.map((court) => (
//             <option key={court.id} value={court.id}>
//               Court {court.id} (Sport ID: {court.sportId}, ₹{court.pricePerHour}
//               /hour)
//             </option>
//           ))}
//         </select>
